...
import { APP_ACTIONS, ICONS, MESSAGES_DATA, PERMISSIONS, ROUTES } from "@constants";
...
export const MessagesPage: React.FC = () => {
  ...
  if (!userPermissions.includes(APP_ACTIONS.messagesPage)) return <Navigate to={ROUTES.not_available} />
  return (
    <section className="pt-20 md:pt-0 h-screen w-full">
      ...
      <div className="w-full h-[calc(100vh-8rem)]">
        <ResizableSlider Left={MessagesLeftPanel} Right={<div className={`relative md:w-2/3 ${selectedMessage === null ? "w-0" : "w-full"}`}>
          <button onClick={handleClick} className={`text-basicGreen text-[30px] absolute left-2 top-3 z-20 md:hidden ${selectedMessage === null ? "hidden" : "block"}`}><ICONS.leftArrow /></button>
          <MessagesRightPanel />
        </div>} />
      </div>
    </section>
