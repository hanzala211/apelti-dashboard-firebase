...
interface TeamTableProps {
  ...
  deleteUserMutation: UseMutationResult<null | undefined, Error, {
    userId: string;
  }, unknown>
}

export const TeamTable: React.FC<TeamTableProps> = ({ teamMembers, isTeamLoading, deleteUserMutation }) => {
  const { userData } = useAuth()
  const { setEditingUser } = useTeam()
  const userPermissions = PERMISSIONS[userData?.role as keyof typeof PERMISSIONS]

  return <div className="overflow-x-auto overflow-y-hidden mt-8">
